...
public:
    ...
    {
        // --- PUT YOUR CODE HERE ---
        float dcx = (float) (x + 0.5);
        dcx = dcx/getResolution().width;
        float dcy = (float) (y + 0.5) ;
        dcy = dcy / getResolution().height;;
        
        float scx = (2 * dcx - 1) * m_aspect;
        float scy = 2 * dcy - 1;
        
        ray.dir = normalize(m_dir * m_focus + sscx * m_xAxis + scy * m_yAxis);
        ray.org = m_pos;
